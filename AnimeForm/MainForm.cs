...
using AnimeForm.Sorted_GroupForms;
using BusinessLogic;
using Model;
...
namespace AnimeForm
{
    public partial class MainForm : Form
    {
        ...
        {
            logic.AddAnime("Attack on Titan", "Action", 9.2, true);
            logic.AddAnime("Naruto", "Adventure", 8.5, true);
            logic.AddAnime("One Piece", "Adventure", 9.0, true);
            logic.AddAnime("Death Note", "Mystery", 8.8, false);
            logic.AddAnime("My Hero Academia", "Action", 8.3, false);
        }
        ...
        {
            var groupForm = new GroupByWatchedForm(logic);
        ...
        {
            // Скрываем главную форму и показываем форму подписки
            this.Hide();
            var subscriptionForm = new SubscriptionForm(this);
